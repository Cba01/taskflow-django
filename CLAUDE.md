...
- Client-side pagination is implemented via `ClayPagination` in `src/components/ui.clay.tsx` (Prev/Next + page count, driven by the `{count, next, previous}` envelope). Used in Dashboard, ProjectDetail (list view), and Notifications; each resets its page to 1 when filters/search change. The Kanban board view doesn't paginate — it loads tasks per status column instead.
- Visual design system lives in `src/components/ui.clay.tsx` ("claymorphism": thick borders, soft double shadows, bouncy entrance/press animations via CSS classes in `index.css`). Color is not tied to a single primary — each `HueKey` (`coral`/`sky`/`mint`/`violet`/`sunshine`/`rose`/`stone`) has a fixed semantic role (create = sunshine, save = mint, destructive = rose, edit = sky, people/identity = violet, neutral = stone) reused consistently across pages. Shared primitives: `ClayButton`, `ClayBadge`, `ClaySelect` (custom listbox — native `<select>` options can't be styled), `ClayModal`/`ClayConfirmDialog`, `ClayField`, `ClayErrorBanner`/`ClayErrorList`, `ClayPagination`, `ClayPageLoading`. Follow this system for any new page rather than reintroducing plain Tailwind grays.
