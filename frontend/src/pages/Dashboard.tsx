...
import { FolderKanban, ListChecks, Plus, Search, Users } from 'lucide-react'
    ...
    <>
      <header className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm font-medium text-slate-500">Los espacios de trabajo donde participas.</p>
        </div>
      ...
      {loading && (
        <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {[0, 1, 2, 3, 4, 5].map((i) => (
            <li
              key={i}
              className="tf-shimmer h-32 rounded-[22px] border-[3px] border-white bg-white"
        ...
        <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {projects.map((project, idx) => {
            ...
            return (
              ...
              >
                <Link to={`/projects/${project.id}`} className={`${CLAY_CARD} flex h-full flex-col items-start gap-4 p-5`}>
